...
The watchdog is deliberately independent of normal command I/O: if a `ContinuousMove` request is accepted by the camera but its HTTP response stalls or is lost, the watchdog can still issue `Stop` at the configured `ptz_safety_timeout_seconds` deadline. Failed safety Stops are retried after `ptz_stop_retry_seconds`. Any new movement-producing command for the same camera—continuous, absolute, relative, or preset—is serialized behind a safety Stop already in flight, preventing an older Stop from racing and cancelling the newer command. Absolute, relative, and preset moves explicitly supersede a pending continuous move. Their payload is validated first; the old continuous-move deadline is retired immediately before transmitting the target command. If that transmission fails ambiguously, safety is re-armed immediately because the camera may still be executing the earlier continuous movement. This prevents both stale Stops after a successful target move and loss of the safety net after a failed one.
